...
export default {
  ...
  computed: {
    currentDirFormatted() {
      ...
      let final_str = this.currentDir.value;
  ...
  mounted() {
    ...
    const dir0 = new TreeNode("~", true);
    const dir1 = new TreeNode("simon-gaufreteau", true);

    const dir2 = new TreeNode("Projects", true);
  ...
  methods: {
    ...
    showTree(node = this.currentDir, level = 0) {
      ...
      this.pushResultToMessageList([
        " ".repeat(level * 4) + node.value + (hasDescendants ? ": {" : ""),
      ]);
      if (hasDescendants) {
